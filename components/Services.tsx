...
const Services: React.FC<ServicesProps> = ({ onNavigate, lang }) => {
  const t = translations[lang];

  const icons = [<MapPin size={24} />, <Camera size={24} />, <Layers size={24} />];
    ...
    <section id="services" className="pt-24 md:pt-40 pb-12 md:pb-20 border-y border-white/5 bg-[#0a0a0a]">
      <div className="container mx-auto px-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 lg:gap-32">
          <div>
            <span className="text-zinc-400 uppercase tracking-[0.5em] text-[10px] mb-4 block">{t.services.badge}</span>
            <h2 className="text-4xl md:text-5xl font-bold tracking-tight mb-12 uppercase">
              {t.services.titleMain} <br />
              <span className="font-serif italic font-normal text-zinc-400">{t.services.titleItalic}</span>
            </h2>
            ...
            {t.services.items.map((service, index) => (
              <div key={service.id} className="group border-b border-white/5 pb-10 last:border-0 cursor-pointer" onClick={() => onNavigate('contact')}>
                <div className="flex justify-between items-center mb-6">
                   ...
                   <div className="opacity-40 group-hover:opacity-100 transition-opacity text-zinc-400">{icons[index]}</div>
                </div>
